...
// prettier-ignore
const regexArray=[/ˣ¹/, /ˣ²/, /ˣ³/, /ˣ⁴/, /ˣ⁵/, /ˣ⁶/, /ˣ⁷/, /ˣ⁸/, /ˣ⁹/, /ˣ¹⁰/, /ˣ²⁰/, /ˣ³⁰/, /ˣ⁴⁰/, /ˣ⁵⁰/, /IPLC/i, /IEPL/i, /核心/, /边缘/, /高级/, /标准/, /实验/, /商宽/, /家宽/, /游戏|game/i, /购物/, /专线/, /LB/, /cloudflare/i, /\budp\b/i, /\bgpt\b/i,/udpn\b/];
// prettier-ignore
const valueArray= [ "1x","2×","3×","4×","5×","6×","7×","8×","9×","10×","20×","30×","40×","50×","IPLC","IEPL","Kern","Edge","Pro","Std","Exp","Biz","Fam","Game","Buy","ZX","LB","CF","UDP","GPT","UDPN"];
...
const rurekey = {
  ...
  孟加拉国: /孟加拉/g,
  捷克: /捷克共和国/g,
  // 台湾: /新台|新北|台(?!.*线)/g, // 移除，避免冲突
  Taiwan: /Taipei/g,
  // 韩国: /春川|韩|首尔/g, // 移除，避免冲突
  Japan: /Tokyo|Osaka/g,
  // 英国: /伦敦/g, // 移除，避免冲突
  India: /Mumbai/g,
  Germany: /Frankfurt/g,
...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // 预处理：清理IP检测脚本可能留下的格式问题
    e.name = e.name
      .replace(/\s*\|\s*/g, ' ')           // 清理竖线分隔符
      .replace(/\s*\.\s*[A-Z]\s*\|\s*/g, ' ') // 清理 ".S |" 这样的格式
      .replace(/\s*(中转|直连|专线|IPLC|IEPL)\s*\|\s*/g, ' $1 ') // 保留重要关键词但清理格式
      .replace(/\s+/g, ' ')                // 合并多个空格
      .trim();
