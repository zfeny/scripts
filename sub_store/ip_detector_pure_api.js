...
function queryIPLocationSync(ip) {
  ...
  try {
    ...
    // 尝试使用同步XMLHttpRequest
    if (typeof XMLHttpRequest !== 'undefined') {
      const xhr = new XMLHttpRequest();
      xhr.open('GET', url, false); // false表示同步请求
      xhr.setRequestHeader('User-Agent', 'Sub-Store-IP-Detector/4.0');
...
function operator(proxies) {
  ...
  console.log(`⚠️ 警告: 只处理IP地址节点，域名节点一律标记为错误`);
  ...
  const results = proxies.map((proxy, index) => {
    try {
      ...
      if (!ip) {
        console.log(`❌ 节点 ${proxy.name} 无法提取IP地址（域名节点）`);
        if (config.fallback) {
      ...
      if (locationInfo) {
        ...
        proxy.detectionMethod = 'api-success';
        
        console.log(`✅ API查询成功: ${originalName} -> ${proxy.name} (${ip} → ${locationInfo.country})`);
        return proxy;
      } else {
        // API查询失败
        ...
        if (config.fallback) {
          proxy.name = `❌ ${proxy.name}`;
          proxy.detectionMethod = 'api-failed';
          return proxy;
        }
        return null;
      }
  ...
  console.log(`\n=== 🏁 检测完成 ===`);
  console.log(`✅ API成功: ${results.filter(p => p.detectionMethod === 'api-success').length} 个节点`);
  console.log(`❌ API失败: ${results.filter(p => p.detectionMethod === 'api-failed').length} 个节点`);
  console.log(`❌ 域名节点: ${results.filter(p => !p.detectionMethod || p.detectionMethod === 'error').length} 个节点`);
...
function showConfig() {
  ...
  console.log('\n=== ⚠️ 重要说明 ===');
  console.log('• 只处理能提取到IP地址的节点');
  console.log('• 域名节点一律标记为 ❌');
  console.log('• API查询失败的节点标记为 ❌');
  console.log('• 不进行任何基于名称或域名的推测');
}
